...
print('나이 : %d' % (age))
print('키01 : %.3f' % (height))
print('키02 : [%10.4f]' % (height)) #전체 10자리 중 소숫점 4자리까지 출력
print('주소 : %s' % (address))

message = '이름은 %s이고, 나이는 %d살입니다.'
print(message % (name, age))  #2가지 이상 변수를 넣을때는 ()안에 넣기 는다
...
message = '[%10s]' % hello # 숫자가 양수이면, 오른쪽 정렬 +우축 정렬
print(message)

message = '[%-10s]' % hello # 숫자가 음수이면, 오른쪽 정렬 -좌측 정렬
print(message)